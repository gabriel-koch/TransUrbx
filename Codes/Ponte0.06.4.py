...
df2 = df2[(df2[u'Veículo'] == '12161')]
#df2 = df2.set_index(['Linha', 'Veiculo'])
#del df1
...
for (vehicle, df_filtred) in df2.groupby(u'Veículo'):
    ...
    for i in range(len(df4)-1):    
        lat1 = df4.get_value(i,3,takeable = True)
        lon1 = df4.get_value(i,4,takeable = True)
        lat2 = df4.get_value(i+1,3,takeable = True)
        lon2 = df4.get_value(i+1,4,takeable = True)
